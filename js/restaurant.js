...
class RestaurantSystem {
    ...
    initializeEventListeners() {
        ...
        document.querySelectorAll('input[name="priceRange"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                this.filters.priceRange = e.target.id.replace('price', '').toLowerCase();
                this.applyFilters();
            });
        });
        ...
        document.querySelectorAll('input[name="rating"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.filters.minRating = ratingId === 'ratingAll' ? 0 : 
                                       ratingId === 'rating4' ? 4.0 : 4.5;
                this.applyFilters();
            });
        });
        ...
        document.querySelectorAll('input[name="deliveryTime"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.filters.maxDeliveryTime = timeId === 'timeAll' ? null :
                                             timeId === 'time30' ? 30 : 45;
                this.applyFilters();
            });
        });
        ...
        document.getElementById('featureFreeDelivery')?.addEventListener('change', (e) => {
    ...
    loadSampleRestaurants() {
        ...
        this.restaurants = [
            ...
            },
            {
                ...
                description: "Sushi autêntico preparado por chefs japoneses. Peixe fresco todos os dias.",
                promo: false,
                open: true,
                freeDelivery: false,
            ...
            },
            {
                ...
                description: "Comida tradicional angolana feita com receitas familiares e ingredientes locais.",
                promo: true,
                open: true,
                freeDelivery: false,
    ...
    applyFilters() {
        ...
        if (this.filters.category) {
            filtered = filtered.filter(restaurant => 
                restaurant.category === this.filters.category
            );
        }
        ...
        if (this.filters.priceRange !== 'all') {
            filtered = filtered.filter(restaurant => {
                if (this.filters.priceRange === 'low') return restaurant.priceRange === 1;
                if (this.filters.priceRange === 'medium') return restaurant.priceRange === 2;
                if (this.filters.priceRange === 'high') return restaurant.priceRange === 3;
                return true;
            });
        }
        ...
        if (this.filters.minRating > 0) {
            filtered = filtered.filter(restaurant => 
                restaurant.rating >= this.filters.minRating
            );
        }
    ...
    displayRestaurants() {
        ...
        this.filteredRestaurants.forEach(restaurant => {
            ...
            col.className = this.currentView === 'grid' ? 'col-md-6 col-xl-4' : 'col-12';
    ...
    createRestaurantCard(restaurant) {
        ...
        const statusClass = restaurant.open ? 
            (restaurant.rating > 4.7 ? 'bg-success' : 'bg-warning text-black') : 'bg-danger';
        const statusText = restaurant.open ? 
            (restaurant.rating > 4.7 ? 'Aberto' : 'Movimentado') : 'Fechado';
                    ...
                    <div class="mt-3">
                        ${restaurant.features.map(feature => `
                            <span class="badge bg-light text-black me-1">
                                <i class="bi bi-${this.getFeatureIcon(feature)}"></i> ${feature}
                            </span>
                        `).join('')}
                    </div>
                </div>
                <div class="card-footer bg-transparent border-0">
                    <a href="restaurante-detalhe.html?id=${restaurant.id}" class="btn btn-danger w-100">
