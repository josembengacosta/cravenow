...
class AcompanharPedidoSystem {
    ...
    loadOrderData() {
        ...
        this.orderData = {
            ...
            items: [
                { name: 'Pizza Margherita Grande', description: 'Massa tradicional, molho de tomate, mussarela', quantity: 1, price: 8500 },
                { name: 'Coca-Cola 2L', description: 'Refrigerante', quantity: 1, price: 1500 },
                { name: 'Molho Extra', description: 'Molho de tomate especial', quantity: 2, price: 500 }
            ]
        };
    ...
    updateLiveData() {
        if (this.orderData && this.orderData.eta > 0) {
            ...
            if (this.orderData.eta <= 5 && !this.orderData.notified5min) {
                this.showNotification('Seu pedido chegará em aproximadamente 5 minutos!');
                this.orderData.notified5min = true;
            }
            
            if (this.orderData.eta <= 2 && !this.orderData.notified2min) {
                this.showNotification('Seu pedido está quase chegando! Prepare-se para receber.');
    ...
    reportProblem() {
        ...
        if (problem) {
            this.showToast('Problema reportado! Entraremos em contacto em breve.', 'success');
        }
    }
            ...
            <div id="${toastId}" class="toast align-items-center text-bg-${typeClass} border-0" role="alert">
                <div class="d-flex">
                    <div class="toast-body">
                        ${message.includes('<') ? message : `<i class="bi ${icon} me-2"></i>${message}`}
                    </div>
