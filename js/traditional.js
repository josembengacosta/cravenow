// traditional-system.js - Sistema de Culinária Tradicional CraveNow
...
class TraditionalSystem {
    ...
    this.dishes = [
      {
        ...
        description:
          "Prato tradicional angolano com galinha, óleo de palma, quiabo e gindungo. Sabor único e aromático.",
        image:
      ...
      },
      {
        ...
        name: "Funge de Bombó",
        description:
          "Massa tradicional feita de farinha de mandioca, acompanhamento essencial da culinária angolana.",
        image:
      ...
      },
      {
        ...
        name: "Mufete",
        description:
          "Prato típico de Luanda com feijão de óleo de palma, peixe grelhado, banana e batata doce.",
        image:
          "https://images.unsplash.com/photo-1563379091339-03246963d96f?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
        price: 3000,
        preparationTime: 60,
        spiceLevel: 2,
        type: "main",
      ...
      },
      {
        ...
        description:
          "Peixe seco cozido com quiabo, óleo de palma e legumes, prato de origem brasileira com adaptação angolana.",
        image:
      ...
      },
      {
        ...
        description:
          "Doce tradicional feito com coco ralado e açúcar, herança da doçaria portuguesa com toque africano.",
        image:
      ...
      },
      {
        ...
        description:
          "Prato tradicional do interior com galinha, amendoim e legumes, de sabor único e cremoso.",
        image:
      ...
      },
      {
        ...
        name: "Pirão",
        description:
          "Massa feita com farinha de milho ou mandioca e caldo de peixe ou carne, acompanhamento tradicional.",
        image:
          "https://images.unsplash.com/photo-1476224203421-9ac39bcb3327?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&q=80",
        price: 600,
        preparationTime: 20,
        spiceLevel: 1,
        type: "side",
        ...
        popularity: 82,
        culturalTip:
          "Deve ter consistência cremosa, nem muito mole nem muito duro",
      ...
      },
      {
        ...
        description:
          "Amendoim torrado com sal, snack tradicional e nutritivo muito popular em Angola.",
        image:
  ...
  handleFilterClick(option) {
    ...
    if (filterType === "special") {
      ...
      if (option.classList.contains("active")) {
        ...
        document
          .querySelectorAll(
            '.filter-option[data-filter^="featured"], .filter-option[data-filter^="vegetarian"], .filter-option[data-filter^="popular"]'
          )
          .forEach((opt) => {
    ...
    setTimeout(() => {
      this.filteredDishes = this.dishes.filter((dish) => {
        ...
        const matchesType =
          ...
          (this.currentFilters.type === "type-dessert" &&
            dish.type === "dessert");

        // Region filter
        const matchesRegion =
          this.currentFilters.region === "region-all" ||
          dish.region === this.currentFilters.region.replace("region-", "");

        // Spice filter
        const matchesSpice =
          this.currentFilters.spice === "spice-all" ||
          (this.currentFilters.spice === "spice-mild" &&
            dish.spiceLevel <= 1) ||
          (this.currentFilters.spice === "spice-medium" &&
            dish.spiceLevel === 2) ||
          (this.currentFilters.spice === "spice-hot" && dish.spiceLevel >= 3);

        // Time filter
        const matchesTime =
          this.currentFilters.time === "time-all" ||
          (this.currentFilters.time === "time-fast" &&
            dish.preparationTime <= 30) ||
          (this.currentFilters.time === "time-medium" &&
            dish.preparationTime > 30 &&
            dish.preparationTime <= 60) ||
          (this.currentFilters.time === "time-slow" &&
            dish.preparationTime > 60);
  ...
  displayDishes() {
    ...
    // Update count
    document.getElementById("dishesCount").textContent =
  ...
  generateDishHTML(dish, index) {
    ...
    return `
            <div class="fade-in-up ${
              dish.isFeatured ? "traditional-featured" : ""
            }" style="animation-delay: ${animationDelay}ms">
                <div class="card traditional-card pattern-border" data-id="${
                  dish.id
                }">
                    ${
                      dish.isFeatured
                    ...
                    <div class="position-relative">
                        <img src="${
                          dish.image
                        }" class="dish-image w-100" alt="${dish.name}">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <h5 class="card-title fw-bold text-traditional">${
                              dish.name
                            }</h5>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <span class="region-badge">${this.getRegionName(
                              dish.region
                            )}</span>
                    ...
                    <div class="card-footer bg-transparent border-0 pt-0">
                        <button class="btn btn-traditional w-100" onclick="traditionalSystem.orderDish(${
                          dish.id
                        })">
  ...
  updateStats() {
    ...
    const traditionalRestaurants = new Set(this.dishes.map((d) => d.restaurant))
      .size;
    const regions = new Set(this.dishes.map((d) => d.region)).size;
    const avgRating = (
      this.dishes.reduce((sum, d) => sum + d.rating, 0) / totalDishes
    ).toFixed(1);

    document.getElementById("traditionalDishes").textContent = totalDishes;
    document.getElementById("traditionalRestaurants").textContent =
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  console.log(
    " Sistema de Culinária Tradicional CraveNow carregado com sucesso!"
  );
});
