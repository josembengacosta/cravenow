...
const LAST_THEME_KEY = "deliveryhj_theme";
const LAST_FILTER_KEY = "deliveryhj_filter";
const LAST_LOCATION_KEY = "deliveryhj_location";
...
function initLocation() {
    ...
    const currentLocationMobile = document.getElementById('currentLocationMobile');
    ...
    const defaultLocation = { province: 'luanda', municipality: 'belas', name: 'Luanda, Belas' };
    const currentLocationData = savedLocation ? JSON.parse(savedLocation) : defaultLocation;
    ...
    function updateLocationText() {
        ...
        if (currentLocation) currentLocation.textContent = currentLocationData.name.split(',')[0];
        if (currentLocationMobile) currentLocationMobile.textContent = currentLocationData.name.split(',')[0];
    }
    ...
    if (saveLocationBtn) {
        saveLocationBtn.addEventListener('click', function() {
            ...
            const municipality = municipalitySelect ? municipalitySelect.value : 'belas';
            const locationName = `${provinceSelect?.options[provinceSelect.selectedIndex]?.text}, ${municipalitySelect?.options[municipalitySelect.selectedIndex]?.text}`;
...
function initNetworkStatus() {
    ...
    function updateNetworkStatus() {
        ...
        if (navigator.onLine) {
            ...
            networkStatus.className = 'alert alert-warning alert-dismissible fade show position-fixed top-0 start-50 translate-middle-x mt-2';
        ...
        card.addEventListener('click', function() {
            ...
            if (restaurant && restaurantModalBody) {
                restaurantModalBody.innerHTML = `
                    <div class="row">
                        <div class="col-md-6">
                            <img src="${restaurant.image}" class="img-fluid rounded" alt="${restaurant.name}">
                        </div>
                        <div class="col-md-6">
                            <h4>${restaurant.name}</h4>
                            <p class="text-muted">${restaurant.description}</p>
                            <div class="mb-3">
                                <span class="badge bg-success me-2"><i class="bi bi-star-fill"></i> ${restaurant.rating}</span>
                                <span class="badge bg-secondary">${restaurant.category}</span>
                            </div>
                            <div class="mb-3">
                                <strong>Entrega:</strong> ${restaurant.deliveryFee}<br>
                                <strong>Tempo:</strong> ${restaurant.deliveryTime}<br>
...
function initFeedbackSystem() {
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', function(e) {
            ...
            if (!rating) {
                showAlert('Por favor, selecione uma avaliação com estrelas.', 'warning');
                return;
            }
            ...
            if (!terms.checked) {
                showAlert('Por favor, aceite os termos para enviar sua avaliação.', 'warning');
                return;
            }
...
function initForms() {
    ...
    if (partnerForm) {
        partnerForm.addEventListener('submit', function(e) {
            e.preventDefault();
            showToast('Pedido de parceria enviado! Entraremos em contacto.', 'success');
            this.reset();
...
function initNewsletterSystem() {
    ...
    if (newsletterForm) {
        ...
        newsletterForm.addEventListener('submit', function(e) {
            ...
            const subscriptions = JSON.parse(localStorage.getItem('deliveryhj_newsletter') || '[]');
            subscriptions.push(formData);
            localStorage.setItem('deliveryhj_newsletter', JSON.stringify(subscriptions));
