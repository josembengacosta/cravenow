...
class RestaurantRegistration {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('prevStep2')?.addEventListener('click', () => this.previousStep());
        document.getElementById('prevStep3')?.addEventListener('click', () => this.previousStep());
        document.getElementById('prevStep4')?.addEventListener('click', () => this.previousStep());

        // Submissão final
        document.getElementById('restaurantRegistrationForm')?.addEventListener('submit', (e) => {
            e.preventDefault();
    ...
    nextStep() {
        if (this.validateCurrentStep()) {
            ...
            this.updateStepDisplay();
            this.scrollToTop();
            this.showToast(`Passo ${this.currentStep - 1} concluído!`, 'success', 3000);
        } else {
    ...
    validateCurrentStep() {
        ...
        if (this.currentStep === 3) {
            const paymentMethods = document.querySelectorAll('#step3 input[type="checkbox"]:checked');
            ...
            col.innerHTML = `
                <div class="image-preview-item position-relative">
                    ...
                    <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 m-1" 
                            onclick="restaurantRegistration.removeImage(this, '${file.name}')">
                        <i class="bi bi-x"></i>
                    </button>
                    <div class="image-info small text-muted mt-1">${this.formatFileSize(file.size)}</div>
                </div>
            `;
    ...
    updateImageCounter() {
        const counter = document.querySelector('.image-counter') || this.createImageCounter();
    ...
    async handleFinalSubmission() {
        ...
        try {
            ...
            if (result.success) {
                this.showToast('🎉 Cadastro enviado com sucesso! Entraremos em contacto dentro de 24h.', 'success', 8000);
                this.resetForm();
            ...
            this.showToast(error.message || 'Erro ao enviar cadastro. Tente novamente.', 'error');
        } finally {
    ...
    async mockBackendSubmit(formData) {
        ...
        return {
            ...
            data: {
                ...
                nextSteps: ['Verificação de documentos', 'Contacto da equipa', 'Ativação da conta']
            }
        };
    }
    ...
    updateRegistrationSummary() {
        ...
        summary.innerHTML = `
            ...
            <div class="row mb-2">
                ...
                <div class="col-7">${this.getCategoryName(data.category) || 'Não informado'}</div>
            </div>
            <div class="row mb-2">
                <div class="col-5"><strong>Localização:</strong></div>
                <div class="col-7">${location.address || 'Não informado'}, ${location.city || ''}</div>
            </div>
            <div class="row">
    ...
    setLoadingState(loading) {
        ...
        if (submitButton) {
            if (loading) {
                ...
                submitButton.innerHTML = '<i class="bi bi-check-lg"></i> Finalizar Cadastro';
    ...
    resetForm() {
        ...
        this.currentStep = 1;
        this.formData = {
            basicInfo: {}, location: {}, schedule: {}, 
            delivery: {}, payments: {}, documents: {}, images: []
        };
        this.updateStepDisplay();
