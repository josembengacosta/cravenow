// search-system.js - Sistema Avançado de Pesquisa para DeliveryHJ
...
class SearchSystem {
    ...
    initializeEventListeners() {
        ...
        document.querySelectorAll('.cursor-pointer').forEach(item => {
            item.addEventListener('click', (e) => {
                const term = e.currentTarget.getAttribute('onclick')?.match(/'([^']+)'/)?.[1];
                if (term) {
                    this.searchByTerm(term);
                }
            });
    ...
    loadSearchData() {
        ...
        this.searchData = [
            // Restaurants
            {
                ...
                type: 'restaurant',
                name: 'Pizza Palace',
                description: 'As melhores pizzas de Luanda, com ingredientes frescos e massa artesanal.',
            ...
            },
            {
                ...
                type: 'restaurant',
                name: 'Burger House',
                description: 'Hambúrgueres artesanais com carne angolana de primeira qualidade.',
            ...
            // Dishes
            {
                ...
                description: 'Pizza com calabresa fatiada, queijo mozzarella e molho de tomate especial.',
            ...
            },
            {
                ...
                type: 'dish',
                name: 'Sushi Salmão',
                description: 'Sushi fresco com salmão norueguês, arroz japonês e alga nori.',
                image: 'https://images.unsplash.com/photo-1579584425555-c3ce17fd4351?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80',
                price: 7500,
            ...
            },
            {
                ...
                description: 'Pão brioche, carne angolana 180g, queijo cheddar, bacon e molho especial.',
            ...
            },
            {
                ...
                description: 'Opções nutritivas e balanceadas para uma alimentação saudável.',
    ...
    searchByCategory(category) {
        ...
        document.getElementById('mainSearchInput').value = this.capitalizeFirstLetter(category);
        this.updateSearchResults();
    ...
    updateSearchResults() {
        ...
        setTimeout(() => {
            this.currentResults = this.searchData.filter(item => {
                ...
                // Filter matching
                const matchesFilter = this.currentFilters.has('all') || 
                    (this.currentFilters.has('restaurant') && item.type === 'restaurant') ||
            ...
            case 'restaurant':
                return `
                    <div class="col-lg-6 mb-4 fade-in-up" style="animation-delay: ${animationDelay}ms">
                        <div class="card result-card result-restaurant cursor-pointer" data-id="${result.id}" data-type="restaurant">
                            <div class="row g-0">
                                <div class="col-md-4">
                                    <img src="${result.image}" class="restaurant-image w-100 h-100" alt="${result.name}">
                                    <span class="result-type-badge badge bg-danger">Restaurante</span>
                                </div>
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start mb-2">
                                            <h5 class="card-title fw-bold">${this.highlightText(result.name)}</h5>
                                            <span class="rating-badge">${result.rating}★</span>
                                        </div>
                                        <p class="card-text text-muted small">${this.highlightText(result.description)}</p>
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="delivery-time">
                                                <i class="bi bi-clock"></i> ${result.deliveryTime}
                                            </span>
                                            ${result.promotion ? '<span class="badge bg-warning text-dark">Promoção</span>' : ''}
                                        </div>
                                        <div class="mt-2">
            ...
            case 'dish':
                return `
                    <div class="col-lg-4 col-md-6 mb-4 fade-in-up" style="animation-delay: ${animationDelay}ms">
                        <div class="card result-card result-dish cursor-pointer" data-id="${result.id}" data-type="dish">
                            <img src="${result.image}" class="dish-image w-100" alt="${result.name}">
                            <span class="result-type-badge badge bg-success">Prato</span>
                            <div class="card-body">
                                <h5 class="card-title fw-bold">${this.highlightText(result.name)}</h5>
                                <p class="card-text text-muted small">${this.highlightText(result.description)}</p>
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="price">${result.price} Kz</span>
                                    <small class="text-muted">${result.restaurant}</small>
                                </div>
                                ${result.promotion ? '<span class="badge bg-warning text-dark mt-2">Promoção Ativa</span>' : ''}
                            </div>
                        </div>
                    </div>
                `;
            ...
            case 'category':
                return `
                    <div class="col-lg-3 col-md-4 col-6 mb-4 fade-in-up" style="animation-delay: ${animationDelay}ms">
                        <div class="card result-card result-category cursor-pointer" data-id="${result.id}" data-type="category">
                            <img src="${result.image}" class="dish-image w-100" alt="${result.name}">
                            <span class="result-type-badge badge bg-warning text-dark">Categoria</span>
                            <div class="card-body text-center">
                                <h6 class="card-title fw-bold">${this.highlightText(result.name)}</h6>
                                <p class="card-text text-muted small">${result.itemCount} itens</p>
                                <div class="mt-2">
                                    ${result.tags.slice(0, 2).map(tag => `<span class="badge bg-light text-dark me-1">${tag}</span>`).join('')}
                                </div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🚀 Sistema de Pesquisa DeliveryHJ carregado com sucesso!');
});