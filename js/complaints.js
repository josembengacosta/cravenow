// complaints-system.js - Sistema de Reclamações DeliveryHJ
...
class ComplaintsSystem {
    ...
    initializeComplaintTypes() {
        ...
        this.selectComplaintType(document.querySelector('.complaint-type-card.active'));
    }
    ...
    openQuickComplaint(type) {
        ...
        const quickComplaints = {
            'late_delivery': {
                ...
                description: 'O pedido está com atraso além do tempo estimado de entrega.',
                solution: 'redelivery'
            },
            'wrong_order': {
                type: 'restaurant',
        ...
        if (complaint) {
            ...
            this.showToast('Formulário preenchido automaticamente! Revise os dados.', 'success');
        }
    }
    ...
    nextStep(step) {
        ...
        document.querySelector('.complaint-card').scrollIntoView({ behavior: 'smooth' });
    }
    ...
    validateStep2() {
        ...
        if (description.length < 20) {
            this.showFieldError('complaintDescription', 'Descrição muito curta (mín. 20 caracteres)');
            return false;
        }
    ...
    updateComplaintData() {
        this.complaintData.orderNumber = document.getElementById('orderNumber').value;
        this.complaintData.urgency = document.getElementById('complaintUrgency').value;
        ...
        this.complaintData.description = document.getElementById('complaintDescription').value;
        this.complaintData.solution = document.getElementById('desiredSolution').value;
    }
    ...
    updateComplaintSummary() {
        ...
        summaryElement.innerHTML = `
            <div class="row">
                <div class="col-md-6">
                    <strong>Tipo:</strong> ${this.getComplaintTypeText(this.complaintData.type)}
                </div>
                <div class="col-md-6">
                    <strong>Urgência:</strong> <span class="badge bg-${this.getUrgencyBadgeColor(this.complaintData.urgency)}">${urgencyText[this.complaintData.urgency]}</span>
                </div>
                ...
                <div class="col-12 mt-2">
                    <strong>Descrição:</strong> ${this.complaintData.description}
                </div>
                <div class="col-12 mt-2">
                    <strong>Solução Desejada:</strong> ${solutionText[this.complaintData.solution]}
                </div>
                <div class="col-12 mt-2">
                    <strong>Evidências:</strong> ${this.complaintData.evidence.length} arquivo(s)
                </div>
            </div>
        `;
    }
    ...
    handleEvidenceUpload(files) {
        ...
        Array.from(files).forEach(file => {
            if (file.size > 5 * 1024 * 1024) { // 5MB limit
                this.showToast(`Arquivo ${file.name} muito grande (máx. 5MB)`, 'warning');
                return;
            }
    ...
    addEvidencePreview(evidence) {
        ...
        item.innerHTML = `
            ${isImage ? `<img src="${evidence.data}" alt="${evidence.name}">` : 
            `<div class="bg-light w-100 h-100 d-flex align-items-center justify-content-center">
                <i class="bi bi-file-earmark-text fs-2 text-muted"></i>
            </div>`}
            <button type="button" class="evidence-remove" onclick="complaintsSystem.removeEvidence('${evidence.name}')">
                <i class="bi bi-x"></i>
            </button>
        `;
    ...
    removeEvidence(fileName) {
        this.complaintData.evidence = this.complaintData.evidence.filter(evidence => evidence.name !== fileName);
    ...
    submitComplaint() {
        ...
        submitButton.innerHTML = '<i class="bi bi-arrow-repeat spinner-border spinner-border-sm"></i> Enviando...';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('🚀 Sistema de Reclamações DeliveryHJ carregado com sucesso!');
});