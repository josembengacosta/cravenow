...
class NavigationSystem {
  ...
  loadCartCount() {
    ...
    const savedCart = localStorage.getItem("CraveNow_cart");
    if (savedCart) {
  ...
  loadUserLocation() {
    const savedLocation = localStorage.getItem("CraveNow_location");
    if (savedLocation) {
  ...
  saveToLocalStorage() {
    localStorage.setItem("CraveNow_location", this.currentLocation);
  }
...
function quickSearch(query) {
  if (query && query.trim() !== "") {
    window.location.href = `pages/restaurant.html?search=${encodeURIComponent(
      query
    )}`;
  }
}
